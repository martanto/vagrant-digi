...
class ImportAlat extends Command
{
    ...
    protected function getAlat()
    {
        return collect($this->alat)->map(function ($item, $key) {
                    $item['scnl'] = $item['station'].'_EHZ_VG_00';
                    return $item;
                });
    }
    ...
    {
        $this->info('Menambahkan Seismometer....');

        $this->getAlat()
            ->each(function ($item, $key) {
                if ($item['jenis'] == 1) {
                    $alat = SeismicStation::updateOrCreate(
                        [
                            'scnl' => $item['scnl'],
                        ],
                        [
